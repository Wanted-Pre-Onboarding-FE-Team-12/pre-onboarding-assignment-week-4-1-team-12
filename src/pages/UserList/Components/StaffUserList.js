...
const StaffUserList = () => {
  ...
  const staffUuidList = staffUser.map(user => {
    return user.uuid;
  });
  const total = staffUuidList.length;

  const staffUserList = userList.filter(
    user => staffUuidList.filter(uuid => uuid === user.uuid).length > 0,
  );
  ...
  return (
    <>
      ...
      {staffUserList?.slice(offset, offset + limit).map(user => {
